...
select count(UNAVAILABLE) from products
where unavailable = -1;

update PRODUCTS  set UNITS_IN_STOCK = NULL
where UNAVAILABLE = -1;
 
select count(UNITS_ON_ORDER) from products
...
/* practicing queries on database */
...
and SUPPLIER_int between 1 and 3
...
where order_int > 10999

union

select order_int,new_disc,  'apply old disc' as disc1 from 
(
select order_int, cast( 0 as int) as new_disc, sum(unit_price*quantity) as total from order_details
group by order_int
having sum(unit_price*quantity) between 0 and 2000
union
select order_int, cast( 0.05 as int) as new_disc, sum(unit_price*quantity) as total from order_details
group by order_int
having sum(unit_price*quantity) between 2001 and 10000
union
select order_int, cast( 0.10 as int) as new_disc, sum(unit_price*quantity) as total from order_details
group by order_int
having sum(unit_price*quantity)between 10000 and 40000
union
select order_int, cast( 0.15 as int) as new_disc, sum(unit_price*quantity) as total from order_details
group by order_int
having sum(unit_price*quantity) between 40001 and 80000
union
select order_int, cast( 20 as int) as new_disc, sum(unit_price*quantity) as total from order_details
...
where order_int between 10000 and 10999
) as Tab3
...
select distinct S.supplier_int,s.company,s.adress,s.phone from suppliers s
inner join products p on p.SUPPLIER_int=s.SUPPLIER_int
where p.category_code = (select category_code from categories where CATEGORY_NAME='Beverages');
...
inner join order_details od on od.order_int=o.ORDER_int
inner join products p on p.product_ref=od.PRODUCT_REF
inner join categories cg on cg.CATEGORY_CODE=p.CATEGORY_CODE
where cg.category_name='desserts' and c.city ='berlin'
group by od.order_int,c.CUSTOMER_CODE
...
from
(
select count(distinct product_ref) as nb_prod,customer_code
from
(select o.customer_code,od.order_int,od.product_ref 
